...
class MigrateCleanCommand extends Command {
  ...
  async performExecute(args = {}) {
    ...
    try {
      ...
      if (error.code === "EACCES") {
        console.error(
          "💡 Tip: Permission denied. You may need to run this command with sudo/administrator privileges to delete files.",
        );
      } else if (error.code === "EBUSY") {
        console.error(
          "💡 Tip: A file is in use by another process. Close any programs that might be using the migration files.",
        );
      } else if (error.code === "EPERM") {
        console.error(
          "💡 Tip: Operation not permitted. This could be a permissions issue or the file might be read-only.",
        );
      }
      ...
      await Promise.all(
        files.map(async (file) => {
          ...
          if (force || (await this.shouldCleanFile(filePath, stats))) {
            ...
            if (verbose) {
              this.progress(
                `Cleaned: ${file} (${this.formatBytes(stats.size)})`,
              );
            }
          }
        }),
  ...
  async cleanTemporaryDatabases(force, verbose) {
    ...
    try {
      ...
      if (!tempDbExists) {
        if (verbose)
          this.progress("Temp databases directory not found, skipping...");
        return { items, size };
      }
      ...
      for (const file of files) {
        if (file.startsWith("test_") || file.startsWith("temp_")) {
          ...
          if (verbose) {
            this.progress(
              `Cleaned temp DB: ${file} (${this.formatBytes(stats.size)})`,
            );
          }
        }
      }
    } catch (error) {
      this.warn("Could not clean temporary databases", {
        error: error.message,
      });
    }
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (force || stats.mtime.getTime() < thirtyDaysAgo) {
          ...
          if (verbose) {
            this.progress(
              `Cleaned old backup: ${file} (${this.formatBytes(stats.size)})`,
            );
          }
        }
      }
      ...
      for (const logFile of possibleLogFiles) {
        try {
          ...
          if (force || stats.size > 10 * 1024 * 1024) {
            ...
            if (verbose) {
              this.progress(
                `Cleaned log: ${logFile} (${this.formatBytes(stats.size)})`,
              );
            }
          }
        } catch (error) {
