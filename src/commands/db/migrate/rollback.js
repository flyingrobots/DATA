...
class MigrateRollbackCommand extends DatabaseCommand {
  ...
  async performExecute(args = {}) {
    ...
    try {
      ...
      this.success(
        `Migration rollback completed to: ${rollbackInfo.migration}`,
      );
  ...
  async getRollbackTarget(target) {
    try {
      ...
      this.warn("Could not determine rollback target", {
        error: error.message,
      });
      return null;
    }
  ...
  async confirmRollback(rollbackInfo) {
    ...
    console.log(
      `Promoted: ${new Date(rollbackInfo.timestamp).toLocaleString()}`,
    );
    ...
    return await this.confirm(
      "Are you absolutely sure you want to rollback?",
      false,
    );
  }
