...
class MigrateStatusCommand extends Command {
  ...
  async performExecute() {
    ...
    try {
      ...
      if (error.code === "ENOENT") {
        // This error code means "File or directory not found"
        console.error(
          "💡 Tip: This might mean your project is not initialized.",
        );
        console.error(
          "   Run `supabase init` to set up a new Supabase project.",
        );
      } else if (error.code === "EACCES") {
        console.error(
          "💡 Tip: Permission denied. Check read access to the supabase/ directory.",
        );
      }
  ...
  async getPendingMigrations() {
    try {
      ...
      this.warn("Could not read migrations directory", {
        error: error.message,
      });
      return [];
    }
  ...
  displayMigrationStatus(stagingStatus, pendingMigrations, lastPromoted) {
    ...
    if (lastPromoted) {
      ...
      console.log(
        `   Date: ${new Date(lastPromoted.timestamp).toLocaleString()}`,
      );
