...
class MigrateTestCommand extends Command {
  ...
  constructor(config = null, logger = null, isProd = false) {
    ...
    if (!serviceRoleKey) {
      throw new Error(
        "SUPABASE_SERVICE_ROLE_KEY environment variable is required for testing",
      );
    }
  ...
  async validateStagedMigration() {
    try {
      ...
      const migrationFile = path.join(
        this.currentMigrationDir,
        "migration.sql",
      );
      await fs.access(migrationFile);
    } catch (error) {
      throw new Error(
        'No staged migration found. Run "data db:migrate:generate" first.',
      );
    }
  }
  ...
  async runTests(options = {}) {
    try {
      ...
      const { data: testFunctions, error: discoverError } =
      ...
      for (const func of testFunctions) {
        ...
        try {
          ...
          const { data: testOutput, error: testError } =
            await this.supabase.rpc("exec_sql", {
          ...
          if (tapResults.failed > 0) {
            this.error(
              `✗ ${functionName}: ${tapResults.failed} test(s) failed`,
            );
          } else {
            this.success(
              `✓ ${functionName}: All ${tapResults.passed} test(s) passed`,
            );
          }
        } catch (error) {
  ...
  reportTestResults(results) {
    ...
    if (results.success) {
      ...
      const failedTests = results.details.filter(
        (d) => d.failed > 0 || d.error,
      );
      if (failedTests.length > 0) {
        console.log("\nFailed Tests:");
        for (const test of failedTests) {
          console.log(
            `  - ${test.function}: ${test.error || `${test.failed} failures`}`,
          );
        }
      }
    }
