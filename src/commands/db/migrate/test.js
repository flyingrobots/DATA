...
class MigrateTestCommand extends Command {
    ...
    try {
      ...
      try {
        ...
        if (testResults.failed > 0) {
          this.error(
            `Migration test failed: ${testResults.failed} test(s) failed`,
          );
        ...
        this.success(
          `Migration test completed successfully: ${testResults.passed} tests passed`,
        );
  ...
  async validateStagedMigration() {
    if (!fs.existsSync(this.currentMigrationDir)) {
      throw new Error(
        'No staged migration found. Run "data compile-migration" first.',
      );
    }
  ...
  async createTestDatabase() {
    ...
    try {
      ...
      await this.processWrapper.execute(
        "createdb",
        [testDbName, "-h", "localhost", "-p", "54332", "-U", "postgres"],
        {
  ...
  async checkPgTapAvailable(testDbUrl) {
    try {
      const result = execSync(
        `psql "${testDbUrl}" -c "SELECT 1 FROM pg_extension WHERE extname = 'pgtap';"`,
        {
          stdio: "pipe",
          encoding: "utf8",
          env: { ...process.env, PGPASSWORD: "postgres" },
      ...
      try {
        ...
        execSync(
          `psql "${testDbUrl}" -c "CREATE EXTENSION IF NOT EXISTS pgtap;"`,
          {
            stdio: "pipe",
            env: { ...process.env, PGPASSWORD: "postgres" },
          },
  ...
  async getAvailableTestFunctions(testDbUrl) {
    try {
      const result = execSync(
        `psql "${testDbUrl}" -c "SELECT routine_name FROM information_schema.routines WHERE routine_schema = 'test' AND routine_name LIKE '%test%' ORDER BY routine_name;"`,
        {
          stdio: "pipe",
          encoding: "utf8",
          env: { ...process.env, PGPASSWORD: "postgres" },
  ...
  async runTestFunction(testDbUrl, functionName) {
    try {
      const result = execSync(
        `psql "${testDbUrl}" -c "SELECT * FROM test.${functionName}();"`,
        {
          stdio: "pipe",
          encoding: "utf8",
          env: { ...process.env, PGPASSWORD: "postgres" },
      ...
      this.progress(
        `Test function ${functionName}: ${passed} passed, ${failed} failed`,
      );
