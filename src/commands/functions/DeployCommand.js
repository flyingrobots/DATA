...
class DeployCommand extends Command {
  ...
  async performExecute(functionNames = null, options = {}) {
    ...
    try {
      ...
      if (failed.length > 0) {
        ...
        this.success(
          `✅ Successfully deployed ${successful.length} function(s)`,
          {
  ...
  async validateEnvironment() {
    ...
    if (!fs.existsSync(path.join(process.cwd(), "supabase", "config.toml"))) {
      throw new Error(
        "Not in a Supabase project root. Run from project root directory.",
      );
    }
  ...
  async validateProductionSecrets() {
    ...
    for (const secret of requiredSecrets) {
      try {
        ...
        const result = execSync(`supabase secrets list --json`, {
          stdio: "pipe",
        });
  ...
  async validateFunction(functionName) {
    ...
    try {
      ...
      if (!content.includes("serve(") && !content.includes("Deno.serve(")) {
        this.warn(
          `Function ${functionName} may not have proper serve() handler`,
        );
      }
      ...
      if (!content.includes("import") && !content.includes("require(")) {
        this.warn(
          `Function ${functionName} has no imports - may be incomplete`,
        );
      }
    } catch (error) {
  ...
  async deployFunction(functionName, options = {}) {
    ...
    try {
      ...
      this.progress(
        `✅ Function ${functionName} deployed successfully (${deployTime}ms)`,
      );
