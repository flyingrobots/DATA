...
class CoverageCommand extends TestCommand {
  ...
  async performExecute(options = {}) {
    ...
    const enforce =
      options.enforce !== undefined
        ? options.enforce
        : testConfig.coverage_enforcement;
    const minCoverage = parseInt(
      options.minCoverage || testConfig.minimum_coverage || "80",
      10,
    );
    ...
    try {
      ...
      const rpcResult = await client.query(
        "SELECT * FROM test.analyze_rpc_coverage()",
      );
      ...
      const policyResult = await client.query(
        "SELECT * FROM test.analyze_policy_coverage()",
      );
      const policyAnalysis = this.analyzer.analyzePolicyCoverage(
        policyResult.rows,
      );
      ...
      const report = this.analyzer.formatCoverageReport(
        rpcAnalysis,
        policyAnalysis,
        summaryResult,
      );
      ...
      const stats = this.analyzer.generateCoverageStats(
        rpcAnalysis,
        policyAnalysis,
      );
      ...
      if (error.code === "ECONNREFUSED") {
        this.error(
          "Database connection refused. Is PostgreSQL running on localhost:54332?",
        );
  ...
  enforceCoverageThresholds(stats, minOverall, minRpc, minRls) {
    ...
    if (stats.rpc && stats.rpc.percentage < minRpc) {
      ...
      this.success(
        chalk.green(
          `✓ RPC function coverage ${stats.rpc.percentage}% meets threshold ${minRpc}%`,
        ),
      );
    }
    ...
    if (passed) {
      ...
      this.error(
        chalk.bold.red(
          `💥 Coverage enforcement failed - ${failures.length} threshold(s) not met`,
        ),
      );
      ...
      failures.forEach((failure) => {
        this.error(
          chalk.red(
            `  • ${failure.type}: ${failure.actual}% < ${failure.expected}%`,
          ),
        );
      });
      ...
      this.progress(
        chalk.yellow(
          "  1. Run: ./build/data test coverage (to see detailed coverage report)",
        ),
      );
      this.progress(
        chalk.yellow(
          "  2. Add missing tests for uncovered RPC functions and RLS policies",
        ),
      );
      this.progress(
        chalk.yellow("  3. Re-run with --enforce to validate improvements"),
      );
    }
