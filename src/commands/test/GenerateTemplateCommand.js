...
} = require("../../lib/testing/TestRequirementAnalyzer");
...
class GenerateTemplateCommand extends TestCommand {
  ...
  async performExecute(options = {}) {
    ...
    try {
      ...
      if (options.migration) {
        requirements = await this.analyzeRequirementsFromMigration(
          options.migration,
        );

        // Filter by type if specified
        if (options.type) {
          requirements = this.filterRequirementsByType(
            requirements,
            options.type,
          );
        }
      ...
      if (result.errors.length > 0) {
        ...
        result.errors.forEach((error) => {
          this.error(
            `Error generating template for ${error.requirement?.name}: ${error.error}`,
          );
        });
      }
  ...
  validateGenerationOptions(options) {
    ...
    if (!options.migration && (!options.type || !options.name)) {
      throw new Error(
        "Either --migration <file> or both --type and --name must be provided",
      );
    }
  ...
  async analyzeRequirementsFromMigration(migrationPath) {
    try {
      ...
      const operations =
        await this.parseMigrationToOperations(migrationContent);

      // Analyze operations to determine test requirements
      const analysis =
        await this.requirementAnalyzer.analyzeOperations(operations);
  ...
  async parseMigrationToOperations(migrationContent) {
    ...
    for (const line of lines) {
      ...
      // Create function operations
        ...
        const match = line.match(
          /CREATE (?:OR REPLACE )?FUNCTION\s+(?:(\w+)\.)?(\w+)\s*\(/i,
        );
        if (match) {
          operations.push({
      ...
      // Trigger operations
        ...
        const match = line.match(
          /CREATE TRIGGER\s+(\w+)\s+.*ON\s+(?:(\w+)\.)?(\w+)/i,
        );
        if (match) {
          operations.push({
            type: "CREATE_TRIGGER",
  ...
  async outputTemplates(templates, outputPath) {
    if (outputPath) {
      ...
      templates.forEach((template) => {
        ...
        console.log(
          `-- Suggested path: ${path.join(template.directory, template.filename)}\n`,
        );
