...
class ValidateCommand extends TestCommand {
  ...
  async loadCache() {
    try {
      ...
      this.info(
        `Loaded validation cache with ${this.validationCache.size} entries`,
      );
    } catch (error) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (tapMatch) {
        ...
        if (!this.pgTapFunctions.has(functionName)) {
          this.addPgTapIssue(
            fileName,
            lineNum,
            `Unknown pgTAP function: tap.${functionName}`,
          );
        }
      }
      ...
      if (line.match(/RETURN\s+NEXT/i) && !line.match(/tap\./i)) {
        this.addPgTapIssue(
          fileName,
          lineNum,
          "RETURN NEXT should typically use tap.* functions",
        );
      }
    }
  }
  ...
  addStructureWarning(fileName, lineNum, message) {
    this.validationResults.structureWarnings.push({
      fileName,
      lineNum,
      message,
    });
  ...
  reportResults() {
    ...
    if (structureWarnings.length > 0) {
      ...
      structureWarnings.forEach((warning) => {
        this.warn(
          `  ${warning.fileName}:${warning.lineNum} - ${warning.message}`,
        );
      });
    }
