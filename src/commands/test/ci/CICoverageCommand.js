...
class CICoverageCommand extends CoverageCommand {
  ...
  async performExecute(options = {}) {
    ...
    try {
      ...
      this.emitCIEvent("coverage_analysis_failed", {
        error: error.message,
        duration,
      });
  ...
  displayCIReport(report) {
    ...
    console.log(
      `OVERALL_COVERAGE: ${summary.overall.percentage}% (${summary.overall.covered}/${summary.overall.total})`,
    );
    console.log(
      `RPC_COVERAGE: ${summary.rpcFunctions.percentage}% (${summary.rpcFunctions.covered}/${summary.rpcFunctions.total})`,
    );
    console.log(
      `RLS_COVERAGE: ${summary.rlsPolicies.percentage}% (${summary.rlsPolicies.covered}/${summary.rlsPolicies.total})`,
    );
  ...
  async writeCIArtifacts(results, report, options) {
    try {
      ...
      console.error(
        `Warning: Could not write coverage artifacts: ${error.message}`,
      );
    }
  }
