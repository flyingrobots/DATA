...
async function cli(argv) {
  ...
  program.hook("preAction", (thisCommand) => {
    ...
    paths = {
      ...
      functionsDir:
        opts.functionsDir || process.env.data_FUNCTIONS_DIR || "./functions",
      // Output paths
      migrationsDir:
        opts.migrationsDir || process.env.data_MIGRATIONS_DIR || "./migrations",
      buildDir: opts.buildDir || process.env.data_BUILD_DIR || "./build",
      reportsDir:
        opts.reportsDir || process.env.data_REPORTS_DIR || "./reports",
    };
  ...
  program
    ...
    .option(
      "--prod",
      "Target production environment (requires confirmation for write operations)",
    )
    .option("--json", "Output results as JSON")
    .option("--no-color", "Disable colored output")
    .option(
      "--config <path>",
      "Path to configuration file (default: .datarc.json)",
    )
  ...
  program
    ...
    .option(
      "--path <path>",
      "Path to initialize project (default: current directory)",
    )
  ...
  db.command("compile")
    .description(
      "Compile SQL sources into migration with optional functions deployment",
    )
    .option(
      "--deploy-functions",
      "Deploy Edge Functions after successful compilation",
    )
    .option(
      "--functions [names...]",
      "Specific functions to deploy (comma-separated)",
    )
    .option(
      "--skip-import-map",
      "Skip using import map in production function deployment",
    )
  ...
  migrate
    ...
    .option(
      "--desired-db <url>",
      "Desired database URL (defaults to compiled SQL)",
    )
    .action(async (options) => {
      const parentOpts = program.opts();
  ...
  test
    ...
    .option(
      "--format <format>",
      "Output format (console, junit, json)",
      "console",
    )
  ...
  test
    ...
    .option(
      "--enforce",
      "Enforce coverage thresholds (exits with code 1 if below threshold)",
    )
    .option(
    ...
    .option(
      "--min-rpc-coverage <percent>",
      "Minimum RPC function coverage percentage",
      "75",
    )
    .option(
      "--min-rls-coverage <percent>",
      "Minimum RLS policy coverage percentage",
      "70",
    )
    .action(async (options) => {
      const parentOpts = program.opts();
  ...
  test
    ...
    .action(async (options) => {
      ...
      if (options.rpc) {
        ...
        console.error(
          "Error: Must specify either --rpc <name> or --rls <name>",
        );
        process.exit(1);
      }
  ...
  test
    ...
    .option(
      "--migration <file>",
      "Migration file to analyze for test requirements",
    )
    .option(
      "--type <type>",
      "Test type (rpc, rls, trigger, constraint, function)",
    )
    .option(
      "--name <name>",
    ...
    .option(
      "--parameters <params>",
      "Comma-separated function parameters for RPC tests",
    )
  ...
  test
    ...
    .option(
      "--output <file>",
      "Output file for validation results (JSON format)",
    )
    .action(async (options) => {
      const parentOpts = program.opts();
