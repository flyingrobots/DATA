...
class ChildProcessWrapper extends EventEmitter {
  ...
  validateCommand(command) {
    ...
    if (!this.allowedCommands.has(baseCommand)) {
      throw new Error(
        `Command '${baseCommand}' is not in the allowed command whitelist`,
      );
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (execOptions.timeout > 0) {
        timeoutHandle = setTimeout(() => {
          timedOut = true;
          this.logger.warn(
            `Process ${pid} timed out after ${execOptions.timeout}ms`,
          );
          ...
          setTimeout(() => {
            if (this.activeProcesses.has(pid)) {
              this.logger.warn(
                `Process ${pid} didn't respond to SIGTERM, sending SIGKILL`,
              );
  ...
  stream(command, args = [], options = {}) {
    ...
    if (execOptions.timeout > 0) {
      timeoutHandle = setTimeout(() => {
        this.logger.warn(
          `Streaming process ${pid} timed out after ${execOptions.timeout}ms`,
        );
        this.killProcess(pid, "SIGTERM");
