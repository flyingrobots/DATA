...
class DiffEngine extends EventEmitter {
  ...
  generateDiff(currentDb, desiredDb, options = {}) {
    if (this.isRunning) {
      throw new Error(
        "DiffEngine is already running. Wait for current operation to complete.",
      );
    }
  ...
  _validateDatabaseInputs(currentDb, desiredDb) {
    if (!currentDb || typeof currentDb !== "object") {
      throw new Error(
        "currentDb parameter must be a valid database connection object",
      );
    }

    if (!desiredDb || typeof desiredDb !== "object") {
      throw new Error(
        "desiredDb parameter must be a valid database connection object",
      );
    }
  }
