    ...
    if (metadata.testing) {
      if (typeof metadata.testing !== "object") {
        ...
        if (
          ...
          (!metadata.testing.tested_at ||
            !this._isValidISO8601(metadata.testing.tested_at))
        ) {
          errors.push(
            "testing.tested_at must be null or valid ISO 8601 date string",
          );
        }

        if (
          metadata.testing.tests_passed !== undefined &&
          (!Number.isInteger(metadata.testing.tests_passed) ||
            metadata.testing.tests_passed < 0)
        ) {
        ...
        if (
          ...
          (!Number.isInteger(metadata.testing.tests_failed) ||
            metadata.testing.tests_failed < 0)
        ) {
    ...
    if (metadata.promotion) {
      if (typeof metadata.promotion !== "object") {
        ...
        if (
          ...
          (!metadata.promotion.promoted_at ||
            !this._isValidISO8601(metadata.promotion.promoted_at))
        ) {
          errors.push(
            "promotion.promoted_at must be null or valid ISO 8601 date string",
          );
        }

        if (
          metadata.promotion.promoted_by !== null &&
          (!metadata.promotion.promoted_by ||
            typeof metadata.promotion.promoted_by !== "string")
        ) {
          errors.push(
            "promotion.promoted_by must be null or a non-empty string",
          );
        }
      }
    }
