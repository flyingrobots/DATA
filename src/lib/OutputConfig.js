...
class OutputConfig {
  ...
  _applyEnvironmentVariables() {
    if (process.env.data_PROJECT_ROOT)
      this.projectRoot = process.env.data_PROJECT_ROOT;
    if (process.env.data_SUPABASE_DIR)
      this.supabaseDir = process.env.data_SUPABASE_DIR;
    if (process.env.data_MIGRATIONS_DIR)
      this.migrationsDir = process.env.data_MIGRATIONS_DIR;
    ...
    if (process.env.data_FUNCTIONS_DIR)
      this.functionsDir = process.env.data_FUNCTIONS_DIR;
  ...
  _loadConfigFile(configPath) {
    ...
    try {
      ...
      console.warn(
        `Warning: Could not parse config file ${configFile}:`,
        error.message,
      );
    }
  }
