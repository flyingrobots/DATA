...
class ASTMigrationEngine extends EventEmitter {
  ...
  async generateMigration(fromSQL, toSQL) {
    ...
    try {
      ...
      migrations.push(
        ...(await this.diffTables(fromSchema.tables, toSchema.tables)),
      );

      // Functions and Triggers
      migrations.push(
        ...(await this.diffFunctions(fromSchema.functions, toSchema.functions)),
      );
      migrations.push(
        ...(await this.diffTriggers(fromSchema.triggers, toSchema.triggers)),
      );

      // RLS Policies (Supabase critical)
      migrations.push(
        ...(await this.diffPolicies(fromSchema.policies, toSchema.policies)),
      );

      // Enums and Custom Types
      migrations.push(
        ...(await this.diffEnums(fromSchema.enums, toSchema.enums)),
      );

      // Indexes
      migrations.push(
        ...(await this.diffIndexes(fromSchema.indexes, toSchema.indexes)),
      );

      // Views
      migrations.push(
        ...(await this.diffViews(fromSchema.views, toSchema.views)),
      );
  ...
  diffTableColumns(tableName, fromTable, toTable) {
    ...
    const fromColumns = new Map(
      fromTable.columns?.map((c) => [c.name, c]) || [],
    );
  ...
  parseFunction(stmt, functions) {
    ...
    // Build signature
    const args =
      stmt.parameters?.map((p) => `${p.name} ${p.type}`).join(", ") || "";
  ...
  extractType(typeName) {
    ...
    if (typeName.names)
      return typeName.names.map((n) => n.String?.str).join(".");
    return "unknown";
  }
  ...
  async diffEnums(fromEnums, toEnums) {
    ...
    for (const [name, toEnum] of toEnums) {
      if (fromEnums.has(name)) {
        ...
        const newValues = toEnum.values.filter(
          (v) => !fromEnum.values.includes(v),
        );
        ...
        const removedValues = fromEnum.values.filter(
          (v) => !toEnum.values.includes(v),
        );
