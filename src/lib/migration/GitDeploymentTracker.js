...
class GitDeploymentTracker extends EventEmitter {
  ...
  async isWorkingTreeClean() {
    try {
      const result = await this.childProcess.execute(
        "git",
        ["status", "--porcelain"],
        {
          cwd: this.gitDir,
      ...
      if (allowedBranches.includes(currentBranch)) {
        try {
          ...
          if (error.message.includes("unknown revision")) {
            ...
            errors.push(
              "Cannot connect to remote repository. Check network connection.",
            );
          } else {
            errors.push(
              `Failed to check remote synchronization: ${error.message}`,
            );
          }
        }
      }
  ...
  async getLastDeploymentTag() {
    try {
      ...
      const result = await this.childProcess.execute(
        "git",
        ["tag", "-l", `${DEPLOYMENT_TAG_PREFIX}*`, "--sort=-version:refname"],
        {
          cwd: this.gitDir,
          timeout: 10000,
        },
      ...
      const hashResult = await this.childProcess.execute(
        "git",
      ...
      let metadata = {};
      try {
        const messageResult = await this.childProcess.execute(
          "git",
  ...
  async getDeploymentHistory(limit = 10) {
    try {
      ...
      for (const tag of tags) {
        ...
        const timestampResult = await this.childProcess.execute(
          "git",
        ...
        const timestamp = new Date(
          parseInt(timestampResult.stdout.trim()) * 1000,
        );
  ...
  async getSQLAtCommit(commitHash) {
    ...
    try {
      ...
      const listResult = await this.childProcess.execute(
        "git",
      ...
      for (const file of sqlFiles) {
        try {
          const fileResult = await this.childProcess.execute(
            "git",
      ...
      throw new Error(
        `Failed to get SQL at commit ${commitHash}: ${error.message}`,
      );
    }
  }
  ...
  async createDeploymentTag(tag, metadata) {
    const fullTag = tag.startsWith(DEPLOYMENT_TAG_PREFIX)
      ? tag
      : `${DEPLOYMENT_TAG_PREFIX}${tag}`;
  ...
  async getChangesBetweenCommits(fromCommit, toCommit = "HEAD") {
    try {
      // Get diff stat
      const statResult = await this.childProcess.execute(
        "git",
      ...
      const filesResult = await this.childProcess.execute(
        "git",
        [
          "diff",
          "--name-status",
      ...
      const summaryMatch = changes.summary.match(
      ...
      throw new Error(
        `Failed to get changes between commits: ${error.message}`,
      );
    }
  }
