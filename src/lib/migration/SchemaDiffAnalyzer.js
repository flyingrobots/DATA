...
class SchemaDiffAnalyzer extends EventEmitter {
  ...
  async analyzeMigration(operations, context = {}) {
    ...
    for (const operation of operations) {
      ...
      if (
        this.compareRiskLevels(opAnalysis.riskLevel, analysis.riskLevel) > 0
      ) {
      ...
      if (
        this.comparePerformanceImpact(
          opAnalysis.performanceImpact,
    ...
    analysis.recommendations.push(
      ...this.generateOverallRecommendations(analysis, context),
    );
  ...
  async analyzeOperation(operation, context) {
    ...
    if (operation.type === "DESTRUCTIVE") {
      analysis.recommendations.push({
        ...
        priority: "HIGH",
        message:
          "Create full database backup before executing destructive operation",
    ...
    if (this.supabasePatterns.rls.test(operation.sql)) {
      if (operation.sql.includes("DROP POLICY")) {
        analysis.warnings.push({
          type: "SECURITY",
          message:
            "Removing RLS policy may expose data - verify security implications",
          severity: "HIGH",
        });
      }
    ...
    if (this.matchesPattern(operation.sql, "CREATE OR REPLACE FUNCTION")) {
      analysis.recommendations.push({
        ...
        priority: "MEDIUM",
        message:
          "Test function changes thoroughly, especially if used in triggers",
        operation: operation.description,
      });
    }
  ...
  assessPerformanceImpact(operation) {
    ...
    if (
      this.matchesPattern(operation.sql, "ALTER TABLE.*ADD COLUMN.*NOT NULL")
    ) {
      return PERFORMANCE_IMPACT.MEDIUM;
    }
  ...
  estimateDuration(operation, context) {
    ...
    // Function/view changes are usually fast
      ...
      this.matchesPattern(operation.sql, "CREATE.*FUNCTION|CREATE.*VIEW")
    ) {
      duration = 0.1;
    }
  ...
  generateOverallRecommendations(analysis, context) {
    ...
    if (analysis.riskLevel === RISK_LEVELS.CRITICAL) {
      recommendations.push({
        ...
        priority: "CRITICAL",
        message:
          "Consider blue-green deployment or maintenance window for critical operations",
      });
    }
    ...
    if (analysis.performanceImpact === PERFORMANCE_IMPACT.HIGH) {
      recommendations.push({
        ...
        priority: "HIGH",
        message:
          "Schedule during low-traffic period due to high performance impact",
      });
    }
    ...
    if (analysis.estimatedDuration > 30) {
      recommendations.push({
        ...
        priority: "MEDIUM",
        message:
          "Monitor migration progress and database performance during execution",
      });
    }
  ...
  extractTableName(sql) {
    ...
    const match = sql.match(
      /(?:CREATE INDEX.*ON|ALTER TABLE|DROP TABLE)\s+([^\s(]+)/i,
    );
