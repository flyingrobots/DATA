...
class CoverageEnforcer extends EventEmitter {
  ...
  async enforce(requirements, coverage, options = {}) {
    this.emit("progress", {
      message: "Starting coverage enforcement analysis...",
    });
    ...
    const report = {
      ...
      coveragePercentage:
        Math.round(
          (comparison.metRequirements.length / filteredRequirements.length) *
            100,
        ) || 0,
      gaps: gaps,
  ...
  compareCoverage(requirements, coverage) {
    this.emit("progress", {
      message: "Comparing requirements against actual coverage...",
    });
  ...
  isGapBlocking(severity, config) {
    switch (config.level) {
      ...
      case ENFORCEMENT_LEVELS.NORMAL:
        return (
          severity === GAP_SEVERITY.CRITICAL || severity === GAP_SEVERITY.HIGH
        );
  ...
  generateRecommendations(gaps, comparison, config) {
    ...
    const criticalGaps = gaps.filter(
      (g) => g.severity === GAP_SEVERITY.CRITICAL,
    );
    ...
    const coveragePercentage =
      Math.round(
        (comparison.metRequirements.length /
          (comparison.metRequirements.length +
            comparison.unmetRequirements.length)) *
          100,
      ) || 0;
    ...
    if (gaps.length > 10) {
      recommendations.push({
        ...
        priority: "MEDIUM",
        message:
          "Consider organizing tests by schema or module for better maintainability",
        gaps: gaps.length,
      });
    }
    ...
    if (
      config.level === ENFORCEMENT_LEVELS.LENIENT &&
      ...
      recommendations.push({
        ...
        priority: "MEDIUM",
        message:
          "Consider using NORMAL or STRICT enforcement for better coverage",
