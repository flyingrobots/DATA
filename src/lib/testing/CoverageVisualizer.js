...
class CoverageVisualizer {
  ...
  formatMatrix(data) {
    ...
    const colWidth = Math.max(
      3,
      Math.max(...data.columns.map((c) => c.length)),
    );
  ...
  _displayOverallStatus(coverage) {
    ...
    console.log(
      "║ " +
        this.colors.dim(
          `Items: ${coverage.covered}/${coverage.total} covered`,
        ) +
        " ".repeat(35) +
        " ║",
    );
  ...
  _displayCategoryBreakdown(categories) {
    ...
    console.log(
      "║ " + this.colors.blue("COVERAGE BY CATEGORY") + " ".repeat(37) + " ║",
    );
    console.log(
      this.colors.frame(
        "╠══════════════════════════════════════════════════════════╣",
  ...
  _displayGaps(gaps) {
    ...
    console.log(
      "║ " + this.colors.red("COVERAGE GAPS DETECTED") + " ".repeat(35) + " ║",
    );
    console.log(
      this.colors.frame(
        "╠══════════════════════════════════════════════════════════╣",
    ...
    Object.entries(groupedGaps).forEach(([category, categoryGaps]) => {
      ...
      categoryGaps.slice(0, 5).forEach((gap) => {
        ...
        const name =
          gap.name.length > 40 ? gap.name.substring(0, 37) + "..." : gap.name;
  ...
  _displaySummary(coverage, gaps) {
    ...
    console.log(
      "║ " + this.colors.blue("ANALYSIS SUMMARY") + " ".repeat(41) + " ║",
    );
    console.log(
      this.colors.frame(
        "╠══════════════════════════════════════════════════════════╣",
    ...
    if (percentage >= 90) {
      recommendation =
        "Coverage is excellent. Maintain current test standards.";
      priorityColor = this.colors.covered;
    } else if (percentage >= 75) {
      recommendation =
        "Good coverage. Consider adding tests for critical gaps.";
