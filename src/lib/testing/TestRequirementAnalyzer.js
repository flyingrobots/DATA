...
class TestRequirementAnalyzer extends EventEmitter {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      warningOperationPriority:
        options.warningOperationPriority || TEST_PRIORITIES.HIGH,
      safeOperationPriority:
        options.safeOperationPriority || TEST_PRIORITIES.MEDIUM,
  ...
  async analyzeOperations(operations, context = {}) {
    ...
    for (let i = 0; i < operations.length; i++) {
      ...
      const requirements = await this.determineTestRequirements(
        operation,
        context,
      );
      ...
      for (const req of requirements) {
        analysis.summary.byType[req.type] =
          (analysis.summary.byType[req.type] || 0) + 1;
    ...
    analysis.suggestions = this._generateTestingSuggestions(
      analysis,
      operations,
      context,
    );
    ...
    analysis.requirements.sort((a, b) =>
      this._comparePriority(a.priority, b.priority),
    );
  ...
  async determineTestRequirements(operation, context = {}) {
    ...
    switch (operationType) {
      case "CREATE_TABLE":
        requirements.push(
          ...this._generateTableCreationTests(operation, target, basePriority),
        );
        break;

      case "DROP_TABLE":
        requirements.push(
          ...this._generateTableDropTests(operation, target, basePriority),
        );
        break;

      case "ALTER_TABLE":
        requirements.push(
          ...this._generateTableAlterationTests(
            operation,
            target,
            basePriority,
          ),
      ...
      case "CREATE_INDEX":
        requirements.push(
          ...this._generateIndexTests(operation, target, basePriority),
        );
        break;

      case "CREATE_FUNCTION":
        requirements.push(
          ...this._generateFunctionTests(operation, target, basePriority),
        );
        break;

      case "CREATE_POLICY":
        requirements.push(
          ...this._generateCreatePolicyTests(operation, target, basePriority),
        );
        break;

      case "ALTER_POLICY":
        requirements.push(
          ...this._generateAlterPolicyTests(operation, target, basePriority),
        );
        break;

      case "DROP_POLICY":
        requirements.push(
          ...this._generateDropPolicyTests(operation, target, basePriority),
        );
        break;

      case "ENABLE_RLS":
        requirements.push(
          ...this._generateEnableRLSTests(operation, target, basePriority),
        );
        break;

      case "DISABLE_RLS":
        requirements.push(
          ...this._generateDisableRLSTests(operation, target, basePriority),
        );
        break;

      case "CREATE_VIEW":
        requirements.push(
          ...this._generateViewTests(operation, target, basePriority),
        );
        break;

      case "CREATE_ENUM":
        requirements.push(
          ...this._generateEnumTests(operation, target, basePriority),
        );
        break;

      case "CREATE_TRIGGER":
        requirements.push(
          ...this._generateTriggerTests(operation, target, basePriority),
        );
        break;
      ...
      default:
        ...
        requirements.push(
          ...this._generateGenericTests(operation, target, basePriority),
        );
    }
    ...
    if (this._requiresSecurityTests(operation)) {
      requirements.push(
        ...this._generateSecurityTests(operation, target, basePriority),
      );
    }
    ...
    if (
      ...
      this._requiresPerformanceTests(operation)
    ) {
      requirements.push(
        ...this._generatePerformanceTests(operation, target, basePriority),
      );
    }
    ...
    for (const req of requirements) {
      ...
      req.metadata =
        req.metadata || this._generateTestMetadata(req, operation, context);
    }
  ...
  _generateTableCreationTests(operation, target, priority) {
    ...
    if (tableStructure.primaryKeys.length > 0) {
      requirements.push({
        ...
        testCases: [
          `has_pk('${target}')`,
          ...tableStructure.primaryKeys.map(
            (pk) => `col_is_pk('${target}', '${pk}')`,
          ),
        ],
      });
    }
    ...
    alterations.addedColumns.forEach((column) => {
      ...
      if (column.hasDefault) {
        ...
        if (column.defaultValue) {
          testCases.push(
            `col_default_is('${target}', '${column.name}', ${column.defaultValue})`,
          );
        }
      }
  ...
  _generateIndexTests(operation, target, priority) {
    ...
    switch (operationType) {
      case "CREATE_INDEX":
        requirements.push(
          ...this._generateCreateIndexTests(operation, target, priority, sql),
        );
        break;

      case "CREATE_UNIQUE_INDEX":
        requirements.push(
          ...this._generateCreateUniqueIndexTests(
            operation,
            target,
            priority,
            sql,
      ...
      case "DROP_INDEX":
        requirements.push(
          ...this._generateDropIndexTests(operation, target, priority, sql),
        );
        break;

      case "ALTER_INDEX":
        requirements.push(
          ...this._generateAlterIndexTests(operation, target, priority, sql),
        );
        break;
    ...
    if (this._requiresIndexPerformanceTests(sql)) {
      requirements.push(
        ...this._generateIndexPerformanceTests(
          operation,
          target,
          priority,
          sql,
    ...
    if (indexDetails.columns && indexDetails.columns.length > 0) {
      requirements.push({
        ...
        testCases: indexDetails.columns.map(
          (col) =>
            `index_is_on('${indexDetails.tableName}', '${target}', '${col}')`,
        ),
        metadata: {
          columns: indexDetails.columns,
          tableName: indexDetails.tableName,
  ...
  _generateFunctionTests(operation, target, priority) {
    ...
    switch (functionOperation) {
      ...
      case "CREATE_OR_REPLACE_FUNCTION":
        requirements.push(
          ...this._generateFunctionCreationTests(
            operation,
            target,
            priority,
            functionMetadata,
      ...
      case "DROP_FUNCTION":
        requirements.push(
          ...this._generateFunctionDropTests(operation, target, priority),
        );
        break;

      case "ALTER_FUNCTION":
        requirements.push(
          ...this._generateFunctionAlterationTests(
            operation,
            target,
            priority,
            functionMetadata,
      ...
      default:
        requirements.push(
          ...this._generateGenericFunctionTests(operation, target, priority),
        );
    }
    ...
    if (this._isSupabaseRpcFunction(operation, functionMetadata)) {
      requirements.push(
        ...this._generateSupabaseRpcTests(
          operation,
          target,
          priority,
          functionMetadata,
    ...
    if (functionMetadata.securityDefiner) {
      requirements.push(
        ...this._generateFunctionSecurityTests(
          operation,
          target,
          priority,
          functionMetadata,
  ...
  _extractFunctionMetadata(operation) {
    ...
    if (paramMatch && paramMatch[1].trim()) {
      ...
      metadata.parameterTypes = paramMatch[1]
        .split(",")
  ...
  _generateFunctionCreationTests(operation, target, priority, metadata) {
    ...
    requirements.push({
      ...
      testCases: [
        ...
        ...(metadata.hasParameters
          ? ["function_args() - parameter validation"]
          : []),
        ...(metadata.language
          ? [`function_lang_is() - language is ${metadata.language}`]
          : []),
        ...(metadata.securityDefiner
          ? ["is_definer() - security definer check"]
          : []),
        ...(metadata.isVolatile !== null
          ? [`volatility_is() - volatility check`]
          : []),
      ],
      metadata: {
    ...
    requirements.push({
      ...
      testCases: [
        ...
        ...(metadata.returnType === "SETOF" ||
        metadata.returnType?.includes("[]")
  ...
  _isSupabaseRpcFunction(operation, metadata) {
    ...
    if (
      ...
      metadata.parameterTypes.every((type) =>
        ["text", "integer", "boolean", "json", "jsonb", "uuid"].includes(
          type.toLowerCase(),
        ),
      )
    ) {
    ...
    for (const role of userRoles) {
      requirements.push({
        ...
        metadata: {
          ...
          testScenarios: this._generateSecurityTestScenarios(
            policyDetails,
            role,
          ),
        },
  ...
  _extractPolicyDetails(sql) {
    ...
    const rolesMatch = sql.match(
      /TO\s+((?:\w+(?:\s*,\s*\w+)*))\s+(?:USING|WITH|$)/i,
    );
    if (rolesMatch) {
    ...
    if (usingStart !== -1) {
      let parenCount = 0;
      let startIdx = sql.indexOf("(", usingStart);
      let endIdx = startIdx;
  ...
  _generateSecurityTestScenarios(policyDetails, role) {
    ...
    for (const command of policyDetails.commands) {
      ...
      scenarios.push({
        role,
        operation: command,
        shouldAllow: this._shouldPolicyAllowOperation(
          policyDetails,
          role,
          command,
        ),
   ...
   * @private
   */
  _generateComprehensiveColumnValidation(
    operation,
    tableName,
    columnName,
    priority,
  ...
  _identifyConstraintType(sql) {
    ...
    if (upperSql.includes("FOREIGN KEY") || upperSql.includes("REFERENCES"))
      return "FOREIGN_KEY";
  ...
  _extractTargetObject(operation) {
    ...
    // Extract table name
    let match = sql.match(
      /(?:CREATE TABLE|DROP TABLE|ALTER TABLE)\s+([^\s(]+)/i,
    );
    if (match) return match[1];

    // Extract index name (handles CREATE, DROP, ALTER INDEX)
    match = sql.match(
      /(?:CREATE(?:\s+UNIQUE)?\s+INDEX|DROP\s+INDEX|ALTER\s+INDEX)\s+([^\s]+)/i,
    );
    if (match) return match[1];
    ...
    match = sql.match(
      /(?:CREATE(?:\s+OR\s+REPLACE)?|DROP|ALTER)\s+FUNCTION\s+([^\s(]+)/i,
    );
    if (match) return match[1];
  ...
  _isHighRiskOperation(operation) {
    const sql = operation.sql || "";
    return (
      this.highRiskPatterns.some((pattern) => pattern.test(sql)) ||
  ...
  _generateTestingSuggestions(analysis, operations, context) {
    ...
    if (analysis.summary.totalRequirements === 0) {
      suggestions.push(
        "No test requirements identified - consider reviewing migration complexity",
      );
    } else {
      suggestions.push(
        `${analysis.summary.totalRequirements} test requirements identified`,
      );
    }

    // Priority-based suggestions
    const criticalTests =
      analysis.summary.byPriority[TEST_PRIORITIES.CRITICAL] || 0;
    if (criticalTests > 0) {
      suggestions.push(
    ...
    if (analysis.estimatedEffort > 8) {
      suggestions.push(
        "Consider parallelizing test implementation due to high effort estimate",
      );
    }
    ...
    if (securityTests > 0) {
      suggestions.push(
        "Security-related changes detected - prioritize RLS and permission tests",
      );
    }
   ...
   */
  generateColumnTestRequirements(
    operation,
    tableName,
    columnName,
    operationType,
    ...
    switch (operationType) {
      case "ADD_COLUMN":
        requirements.push(
          ...this._generateColumnAdditionTests(
            operation,
            tableName,
            columnName,
            priority,
      ...
      case "ALTER_TYPE":
        requirements.push(
          ...this._generateColumnTypeChangeTests(
            operation,
            tableName,
            columnName,
            priority,
      ...
      case "SET_NOT_NULL":
        requirements.push(
          ...this._generateColumnNotNullTests(
            operation,
            tableName,
            columnName,
            priority,
      ...
      case "DROP_NOT_NULL":
        requirements.push(
          ...this._generateColumnNullableTests(
            operation,
            tableName,
            columnName,
            priority,
      ...
      case "SET_DEFAULT":
        requirements.push(
          ...this._generateColumnSetDefaultTests(
            operation,
            tableName,
            columnName,
            priority,
      ...
      case "DROP_DEFAULT":
        requirements.push(
          ...this._generateColumnDropDefaultTests(
            operation,
            tableName,
            columnName,
            priority,
  ...
  _parseTableStructure(sql) {
    ...
    try {
      ...
      const tableDefMatch = sql.match(
        /CREATE TABLE\s+[^\s(]+\s*\(([\s\S]*?)\)(?:\s*;|\s*$)/i,
      );
      if (!tableDefMatch) {
        return structure;
      }
      ...
      for (const item of items) {
        ...
        if (cleanItem.toUpperCase().startsWith("PRIMARY KEY")) {
          ...
          const uniqueMatch = cleanItem.match(
            /UNIQUE\s*(?:\(\s*([^)]+)\s*\))?/i,
          );
          if (uniqueMatch) {
            structure.uniqueConstraints.push({
              name: `unique_${uniqueMatch[1] || "constraint"}`,
              columns: uniqueMatch[1]
                ? uniqueMatch[1].split(",").map((c) => c.trim())
                : [],
            });
          }
          ...
          const constraintMatch = cleanItem.match(
            /CONSTRAINT\s+([^\s]+)\s+(.*)/i,
          );
          if (constraintMatch) {
            ...
            if (constraintDef.toUpperCase().startsWith("PRIMARY KEY")) {
              const pkMatch = constraintDef.match(
                /PRIMARY KEY\s*\(\s*([^)]+)\s*\)/i,
              );
              if (pkMatch) {
                structure.primaryKeys = pkMatch[1]
              ...
              const uniqueMatch = constraintDef.match(
                /UNIQUE\s*\(\s*([^)]+)\s*\)/i,
              );
              if (uniqueMatch) {
                structure.uniqueConstraints.push({
  ...
  _parseTableAlterations(sql, tableName) {
    ...
    try {
      ...
      while ((addMatch = addColumnRegex.exec(upperSql)) !== null) {
        ...
        alterations.addedColumns.push(
          this._parseColumnDefinition(`${columnName} ${columnDef}`),
        );
      }
      ...
      const renameColumnRegex =
        /RENAME\s+(?:COLUMN\s+)?([^\s]+)\s+TO\s+([^\s,;]+)/gi;
      let renameColMatch;
      ...
      while (
        (dropConstraintMatch = dropConstraintRegex.exec(upperSql)) !== null
      ) {
    ...
    try {
      ...
      if (columnName) {
        ...
        if (columnIndex !== -1) {
          ...
          const beforeParts = beforeColumn.trim()
            ? beforeColumn.trim().split(/\s+/)
            : [];
          ...
          if (
            parts[0].toUpperCase() === "ADD" &&
            parts[1].toUpperCase() === "COLUMN"
          ) {
            nameIndex = 2;
            typeIndex = 3;
      ...
      const referencesMatch = defString.match(
        /REFERENCES\s+([^\s(]+)(?:\s*\(\s*([^)]+)\s*\))?/i,
      );
      if (referencesMatch) {
        column.foreignKey = {
      ...
      console.warn(
        "Failed to parse column definition:",
        columnDef,
        error.message,
      );
      return null;
  ...
  _parseColumnType(parts, typeIndex) {
    ...
    if (
      typeIndex + 1 < parts.length &&
      parts[typeIndex + 1].match(/^\(\d+(?:,\d+)?\)$/)
    ) {
  ...
  _generateTriggerAlterationTests(operation, target, priority) {
    ...
    if (
      sql.toUpperCase().includes("ENABLE") ||
      sql.toUpperCase().includes("DISABLE")
    ) {
  ...
  _generateEventTriggerTests(operation, target, priority) {
    ...
    requirements.push({
      ...
      metadata: {
        ddlOperationsTested: eventDetails.events || [
          "ddl_command_start",
          "ddl_command_end",
        ],
  ...
  _parseTriggerDetails(sql) {
    ...
    const functionMatch = sql.match(
      /EXECUTE\s+(?:PROCEDURE\s+|FUNCTION\s+)?([^\s(]+)/i,
    );
    ...
    else if (sql.toUpperCase().includes("INSTEAD OF"))
    ...
    // Extract level
    details.level = sql.toUpperCase().includes("FOR EACH ROW")
      ? "ROW"
      : "STATEMENT";
    ...
    details.isConstraintTrigger = sql
      .toUpperCase()
      .includes("CONSTRAINT TRIGGER");
    details.deferrable = sql.toUpperCase().includes("DEFERRABLE");
    details.initiallyDeferred = sql
      .toUpperCase()
      .includes("INITIALLY DEFERRED");
  ...
  _generateTriggerBehaviorTests(triggerDetails) {
    ...
    (triggerDetails.events || []).forEach((event) => {
      scenarios.push({
        ...
        operation: event,
        expectedResult:
          "Trigger function executes and modifies data as expected",
      });
  ...
  aggregateRequirements(requirementsList) {
    ...
    this.emit("progress", {
      message: "Aggregating test requirements from multiple operations...",
    });
    ...
    const aggregatedRequirements =
      this._extractAggregatedRequirements(aggregationState);
  ...
  _trackRelatedObjects(requirement, aggregationState) {
    ...
    if (requirement.metadata) {
      ...
      if (
        requirement.type === TEST_TYPES.INDEX &&
        requirement.metadata.tableName
      ) {
        objectInfo.dependencies.add(requirement.metadata.tableName);
        this._ensureRelatedObject(
          requirement.metadata.tableName,
          "TABLE",
          aggregationState,
        );
        aggregationState.relatedObjects
          .get(requirement.metadata.tableName)
      ...
      if (requirement.metadata.referencedTable) {
        ...
        this._ensureRelatedObject(
          requirement.metadata.referencedTable,
          "TABLE",
          aggregationState,
        );
        aggregationState.relatedObjects
      ...
      if (
        requirement.type === TEST_TYPES.RLS &&
        requirement.metadata.tableName
      ) {
        objectInfo.dependencies.add(requirement.metadata.tableName);
        this._ensureRelatedObject(
  ...
  _mergeDuplicateRequirements(aggregationState) {
    for (const [targetKey, requirements] of aggregationState.targetGroups) {
      ...
      const descriptionGroups =
        this._groupByDescriptionSimilarity(requirements);
  ...
  _areRequirementsSimilar(req1, req2) {
    ...
    const commonKeywords = desc1Keywords.filter((k) =>
      desc2Keywords.includes(k),
    );

    // At least 50% keyword overlap
    return (
      commonKeywords.length >=
      Math.max(desc1Keywords.length, desc2Keywords.length) * 0.5
    );
  }
  ...
  _mergeRequirementGroup(group) {
    ...
    // Merge metadata
    const mergedMetadata = this._mergeMetadata(
      group.map((r) => r.metadata).filter(Boolean),
    );
  ...
  _resolveConflictsAndMergeRelated(aggregationState) {
    ...
    for (const cascade of aggregationState.cascadingChanges) {
      if (cascade.impact === "HIGH") {
        this._elevatePriorityForTarget(
          cascade.target,
          TEST_PRIORITIES.HIGH,
          aggregationState,
        );
  ...
  _extractAggregatedRequirements(aggregationState) {
    ...
    for (const [
      _targetKey,
      targetRequirements,
    ] of aggregationState.targetGroups) {
  ...
  _generateAggregationSummary(
    ...
    for (const req of aggregatedRequirements) {
      ...
      priorityDistribution[req.priority] =
        (priorityDistribution[req.priority] || 0) + 1;
    ...
    return {
      ...
      deduplicationRate:
        originalCount > 0
          ? ((duplicatesRemoved / originalCount) * 100).toFixed(1)
          : 0,
      priorityDistribution,
      typeDistribution,
      targetCoverage,
