...
module.exports = {
  ...
  calculateSeverityScore(gap) {
    ...
    const missingAssertionsWeight = Math.min(
      gap.missingAssertions.length * 5,
      30,
    );
    const typeWeight =
      gap.objectType === "function" || gap.objectType === "policy" ? 20 : 10;
    const criticalPathBonus = gap.metadata?.isCriticalPath ? 10 : 0;

    return Math.min(
      baseScore + missingAssertionsWeight + typeWeight + criticalPathBonus,
      100,
    );
  },
