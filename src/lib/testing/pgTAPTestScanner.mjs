...
class pgTAPTestScanner extends EventEmitter {
  ...
  _initializeAssertionPatterns() {
    ...
    patterns.set(
      "has_schema",
      /SELECT\s+has_schema\s*\(\s*['"`]([^'"`]+)['"`]\s*\)/gi,
    );
    patterns.set(
      "hasnt_schema",
      /SELECT\s+hasnt_schema\s*\(\s*['"`]([^'"`]+)['"`]\s*\)/gi,
    );
  ...
  async scanDirectory(testsDir) {
    ...
    try {
      ...
      for (let i = 0; i < testFiles.length; i++) {
        ...
        try {
          ...
          const errorEvent = ErrorEvent.fromError(
            error,
            `Failed to parse test file: ${filePath}`,
          );
          this.emit("error", {
      ...
      const errorEvent = ErrorEvent.fromError(
        error,
        `Failed to scan tests directory: ${testsDir}`,
      );
      this.emit("error", {
        message: errorEvent.message,
        error: errorEvent.error,
    ...
    for (const [assertionType, pattern] of this.assertionPatterns) {
      ...
      while ((match = pattern.exec(processedSql)) !== null) {
        ...
        if (
          ...
          assertion.functionMetadata = this._extractFunctionMetadata(
            assertionType,
            parameters,
          );
        }
        ...
        if (assertionType.includes("table")) {
          const metadata = this._extractAssertionMetadata(
            assertionType,
            parameters,
          );
        ...
        if (
          ...
          assertion.policyMetadata = this._extractPolicyMetadata(
            assertionType,
            parameters,
          );
        }
      ...
      for (const entry of sortedEntries) {
        ...
        try {
          ...
          if (
            ...
            !this._shouldIncludeHidden(relativePath)
          ) {
            continue;
          }
          ...
          if (entry.isDirectory()) {
            ...
            const subFiles = await this._findTestFiles(
              fullPath,
              depth + 1,
              testsRootDir,
            );
  ...
  _shouldIncludeHidden(relativePath) {
    ...
    return this.options.includePatterns.some((pattern) => {
      ...
      return (
        pattern.includes(".") && this._matchesPattern(relativePath, pattern)
      );
    });
  }
  ...
  _matchesPattern(filePath, pattern) {
    ...
    try {
      ...
      console.warn(
        `Invalid regex pattern generated from ${normalizedPattern}: ${error.message}`,
      );
  ...
  _determineTarget(assertionType, parameters) {
    ...
    if (assertionType.includes("column") || assertionType.startsWith("col_")) {
      ...
      if (
        assertionType === "col_type_is" ||
        assertionType === "col_default_is"
      ) {
        if (parameters.length >= 4) {
    ...
    if (
      ...
      if (
        assertionType === "has_function" ||
        assertionType === "hasnt_function"
      ) {
      ...
      // is_definer('function_name', ARRAY['type1', 'type2'])
        ...
        assertionType === "is_definer" ||
        assertionType === "isnt_definer"
      ) {
        if (parameters.length >= 2 && !parameters[1].includes("'")) {
          // Schema and function name provided
          return `${parameters[0]}.${parameters[1]}`;
    ...
    if (assertionType.includes("index") || assertionType.includes("unique")) {
      ...
      if (assertionType === "has_index" || assertionType === "hasnt_index") {
        ...
        assertionType === "has_unique" ||
        assertionType === "hasnt_unique"
      ) {
    ...
    if (assertionType.includes("trigger")) {
      if (
        assertionType === "has_trigger" ||
        assertionType === "hasnt_trigger"
      ) {
        // has_trigger('table', 'trigger_name') or has_trigger('schema', 'table', 'trigger_name')
        ...
        if (parameters.length === 2) {
          ...
          if (
            ...
            (parameters[2].toLowerCase().includes("trigger") &&
  ...
  _addToCoverageMap(
    ...
    if (category === "tables" && assertion) {
      ...
      if (
        ...
        !tableInfo.descriptions.includes(assertion.description)
      ) {
      ...
      async (batch, batchIndex) => {
        ...
        if (this.streamingDB) {
          for (const testFile of batch) {
            if (
              !this.streamingDB.addObject("files", testFile.filePath, testFile)
            ) {
              this.emit("warning", {
                type: "memory_limit",
  ...
  _processFileForDatabase(testFile, database) {
    ...
    for (const assertion of testFile.assertions) {
      ...
      const typeCount =
        database.assertionCounts.byType.get(assertion.type) || 0;
  ...
  getCoverageByObject(objectType, objectName) {
    ...
    const totalPossibleAssertions = this._estimateMaxAssertions(
      normalizedType,
      objectName,
    );
    const coveragePercentage =
      totalPossibleAssertions > 0
        ? Math.round(
            (objectCoverage.assertions.length / totalPossibleAssertions) * 100,
          )
        : 100;
  ...
  getCoverageStatistics() {
    ...
    const stats = {
      ...
      quality: {
        averageAssertionsPerObject:
          db.totalAssertions / this._getTotalIndexedObjects(db),
  ...
  _indexAssertionInDatabase(database, assertion, testFile) {
    ...
    if (assertion.policyMetadata) {
      objectEntry.metadata.policy = {
        ...objectEntry.metadata.policy,
        ...assertion.policyMetadata,
      };
    }
  ...
  _parseObjectFromTarget(assertionType, target) {
    ...
    if (assertionType.includes("table") || assertionType.includes("rls")) {
      ...
      assertionType.includes("column") ||
      assertionType.includes("col_")
    ) {
      objectType = "columns";
    ...
    for (const [objectType, objectMap] of Object.entries(database.objects)) {
      for (const [objectName, objectData] of objectMap.entries()) {
        ...
        if (assertionCount === 0) {
          ...
          assertionCount < this._getMinimumAssertionThreshold(objectType)
        ) {
  ...
  _getTotalIndexedObjects(database) {
    return Object.values(database.objects).reduce(
      (total, objectMap) => total + objectMap.size,
      0,
    );
  }
      ...
      switch (objectType) {
        ...
        default:
          recommendations.push(
            `Add test coverage for ${objectType}: ${objectName}`,
          );
      }
    }
  ...
  _getTopTestedObjects(limit = 10) {
    ...
    for (const [objectType, objectMap] of Object.entries(
      this.coverageDatabase.objects,
    )) {
      for (const [objectName, objectData] of objectMap.entries()) {
        objectStats.push({
          objectType,
  ...
  _buildCoverageReport(objectTypes, includeDetails) {
    ...
    const typesToInclude =
      objectTypes || Object.keys(this.coverageDatabase.objects);
  ...
  _formatReportAsCsv(report) {
    const lines = [
      "Object Type,Object Name,Assertion Count,Assertion Types,Test Files,Coverage %",
    ];

    for (const [objectType, objects] of Object.entries(report.coverage)) {
      for (const [objectName, data] of Object.entries(objects)) {
        const maxAssertions = this._estimateMaxAssertions(
          objectType,
          objectName,
        );
        const coverage =
...
<body>
  ...
  _checkMemoryUsage() {
    ...
    this.memoryState.maxUsageMB = Math.max(
      this.memoryState.maxUsageMB,
      usage.heapUsed,
    );
  ...
  _limitObjectAccumulation() {
    ...
    Object.keys(this.coverageMap).forEach((type) => {
      ...
      if (objectKeys.length > this.options.maxObjectsPerType) {
        ...
        const toKeep = objectKeys.slice(
          -Math.floor(this.options.maxObjectsPerType * 0.8),
        );
        const newObjects = {};
