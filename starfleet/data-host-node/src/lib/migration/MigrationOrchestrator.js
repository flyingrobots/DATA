...
class MigrationOrchestrator extends SupabaseCommand {
  constructor(options = {}) {
    ...
    this.migrationsDir = options.migrationsDir || process.env.ARCHY_MIGRATIONS_DIR || './migrations';
    this.functionsDir = options.functionsDir || process.env.ARCHY_FUNCTIONS_DIR || './functions';
    this.testsDir = options.testsDir || process.env.ARCHY_TESTS_DIR || './tests';
    ...
    this.coverageOrchestrator = options.coverageOrchestrator || new TestCoverageOrchestrator({
  ...
  setupEngineEventHandlers() {
    ...
    this.astEngine.on('start', (data) => this.progress('AST Engine: ' + data.message, data));
    this.astEngine.on('progress', (data) => this.progress('AST Engine: ' + data.message, data));
    
    // Forward coverage orchestrator events
    this.coverageOrchestrator.on('progress', (data) => this.progress('Coverage: ' + data.message, data));
    this.coverageOrchestrator.on('warning', (data) => this.warn('Coverage: ' + data.message, data));
    this.coverageOrchestrator.on('enforcement_failed', (data) => this.error('Coverage enforcement failed', data));
    this.coverageOrchestrator.on('enforcement_bypassed', (data) => this.warn('Coverage enforcement bypassed', data));
    this.astEngine.on('warning', (data) => this.warn('AST Engine: ' + data.message, data));
    this.astEngine.on('error', (data) => this.error('AST Engine: ' + data.message, data.error, data));
    this.astEngine.on('complete', (data) => this.progress('AST Engine: ' + data.message, data));
    
    // Forward Git tracker events
    this.gitTracker.on('progress', (data) => this.progress('Git Tracker: ' + data.message, data));
    this.gitTracker.on('warning', (data) => this.warn('Git Tracker: ' + data.message, data));
    this.gitTracker.on('error', (data) => this.error('Git Tracker: ' + data.message, data.error, data));
    
    // Forward diff analyzer events
    this.diffAnalyzer.on('progress', (data) => this.progress('Diff Analyzer: ' + data.message, data));
    this.diffAnalyzer.on('warning', (data) => this.warn('Diff Analyzer: ' + data.message, data));
    this.diffAnalyzer.on('complete', (data) => this.progress('Diff Analyzer: ' + data.message, data));
  }
  ...
  async performExecute(options = {}) {
    ...
    try {
      ...
      if (!this.skipCoverage && analysisResult.operations.length > 0) {
        ...
        const coverageResult = await this.checkTestCoverage(analysisResult.operations);
      ...
      if (!this.dryRun) {
        ...
        const migrationResult = await this.executeMigration(analysisResult.operations);
        ...
        return { success: true, dryRun: true, operations: analysisResult.operations };
      }
      ...
      if (this.currentPhase === PHASES.FUNCTIONS || 
        ...
        try {
          ...
          throw new Error(`Deployment failed and rollback failed: ${error.message}. Manual intervention required.`);
        }
      }
  ...
  async checkTestCoverage(operations) {
    ...
    try {
      const coverageResult = await this.coverageOrchestrator.checkCoverage(operations, {
      ...
      if (coverageResult.passed) {
        this.success(`Test coverage check passed (${coverageResult.coveragePercentage}% coverage)`);
      } else if (coverageResult.bypassReason) {
        this.warn(`Test coverage bypassed: ${coverageResult.bypassReason}`);
      } else {
        this.warn(`Test coverage insufficient: ${coverageResult.coveragePercentage}%`);
        this.warn(`Missing tests for ${coverageResult.gaps.length} requirements`);
        
        // Show top 5 gaps
        const topGaps = coverageResult.gaps.slice(0, 5);
        for (const gap of topGaps) {
          this.warn(`  - ${gap.requirement.type}: ${gap.requirement.name} (${gap.requirement.priority})`);
        }
  ...
  async analyzeChanges(gitState) {
    ...
    const operations = await this.astEngine.generateMigration(previousSQL, currentSQL);
    ...
    this.progress('Schema analysis complete', {
      ...
      destructiveOps: operations.filter(op => op.type === 'DESTRUCTIVE').length,
  ...
  async previewChanges(analysisResult) {
    ...
    this.emit('preview', {
      summary: {
        ...
        warningOperations: operations.filter(op => op.type === 'WARNING').length,
        destructiveOperations: operations.filter(op => op.type === 'DESTRUCTIVE').length,
        riskLevel: analysis.riskLevel
      },
  ...
  async executeMigration(operations) {
    ...
    try {
      ...
      for (let i = 0; i < operations.length; i++) {
        ...
        this.progress(`Executing operation ${i + 1}/${operations.length}: ${operation.description}`);
        ...
        try {
          ...
          throw new Error(`Operation failed: ${operation.description}. Error: ${error.message}`);
        }
      }
  ...
  async deployFunctions() {
    ...
    try {
      ...
      const deployResult = await this.childProcess.execute('supabase', [
        'functions', 'deploy',
        '--project-ref', this.getProjectRef()
      ], {
        cwd: process.cwd(),
  ...
  async rollback(options = {}) {
    ...
    try {
      ...
      const targetSQL = await this.gitTracker.getSQLAtCommit(lastDeployment.hash);
      const currentSQL = await this.loadCurrentSQL();
      
      // Generate reverse migration
      const rollbackOps = await this.astEngine.generateMigration(currentSQL, targetSQL);
      
      if (!options.automatic) {
        this.warn(`Rolling back ${rollbackOps.length} operations to deployment ${lastDeployment.tag}`);
      ...
      await this.tagDeployment({ id: 'rollback', operations: rollbackOps.length });
      ...
      throw new Error(`Rollback failed: ${error.message}. Manual intervention required.`);
    }
  }
