  ...
  describe("Complete Migration Workflow", () => {
    it("should handle full migration lifecycle from SQL to deployment", async () => {
      ...
      const migrations = await fs
        .readdir(path.join(tempDir, "migrations"))
        .catch(() => []);
      ...
      try {
        ...
        if (newMigrations.length > migrations.length) {
          ...
          const newMigrationDir = path.join(
            tempDir,
            "migrations",
            newMigrations[newMigrations.length - 1],
          );
          const newUpSql = await fs.readFile(
            path.join(newMigrationDir, "up.sql"),
            "utf8",
          );
          ...
          console.log(
            "✅ Confirmed: Generated short-path incremental migration",
          );
        } else {
  ...
  describe("Test Coverage Enforcement in Workflow", () => {
    it("should enforce test coverage during migration workflow", async () => {
      ...
      if (testFiles.length > 0) {
        ...
        console.log(
          "🗑️ Removed test file to create coverage gap:",
          testFiles[0],
        );
      }
