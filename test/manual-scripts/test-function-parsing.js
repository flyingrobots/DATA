...
runTest("has_function with schema and function name", () => {
  ...
  assertEquals(
    assertions[0].functionMetadata.schema,
    "public",
...
runTest("function_returns parsing", () => {
  ...
  assertEquals(
    assertions[0].type,
    "function_returns",
    "Should be function_returns type",
  );
  assertEquals(
    assertions[0].target,
    "user_count",
  ...
  assertEquals(
    assertions[0].functionMetadata.returnType,
    "integer",
...
runTest("function_lang_is parsing", () => {
  ...
  assertEquals(
    assertions[0].type,
    "function_lang_is",
    "Should be function_lang_is type",
  );
  assertEquals(
    assertions[0].target,
    "user_count",
  ...
  assertEquals(
    assertions[0].functionMetadata.language,
    "plpgsql",
...
runTest("volatility_is parsing", () => {
  ...
  assertEquals(
    assertions[0].functionMetadata.volatility,
    "immutable",
    "Should extract volatility",
  );
});
...
runTest("function_privs_are parsing", () => {
  const sql =
    "SELECT function_privs_are('calc_func', 'app_user', ARRAY['EXECUTE']);";
  const assertions = scanner.extractAssertions(sql);
  ...
  assertEquals(
    assertions[0].type,
    "function_privs_are",
    "Should be function_privs_are type",
  );
  ...
  assertEquals(
    assertions[0].functionMetadata.role,
    "app_user",
    "Should extract role",
  );
  assertEquals(
    assertions[0].functionMetadata.privileges,
    ["EXECUTE"],
    "Should extract privileges",
  );
});
