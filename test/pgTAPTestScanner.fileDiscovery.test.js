...
describe("pgTAPTestScanner File Discovery", () => {
  ...
  describe("Basic file discovery", () => {
    ...
    it("should find test files recursively", async () => {
      ...
      await writeFile(
        join(subDir, "nested.sql"),
        "SELECT has_table('nested');",
      );
  ...
  describe("File extension configuration", () => {
    it("should support custom file extensions", async () => {
      ...
      await writeFile(
        join(tempDir, "test2.test.sql"),
        "SELECT has_table('test2');",
      );
      await writeFile(
        join(tempDir, "test3.pgtap"),
        "SELECT has_table('test3');",
      );
  ...
  describe("Include/exclude patterns", () => {
    ...
    it("should respect include patterns", async () => {
      ...
      await writeFile(
        join(tempDir, "unit-test1.sql"),
        "SELECT has_table('test1');",
      );
      await writeFile(
        join(tempDir, "integration-test.sql"),
        "SELECT has_table('test2');",
      );
      await writeFile(
        join(tempDir, "unit-test2.sql"),
        "SELECT has_table('test3');",
      );
  ...
  describe("Depth limiting", () => {
    it("should respect maximum depth setting", async () => {
      ...
      await writeFile(join(tempDir, "root.sql"), "SELECT has_table('root');");
      await writeFile(
        join(level1, "level1.sql"),
        "SELECT has_table('level1');",
      );
      await writeFile(
        join(level2, "level2.sql"),
        "SELECT has_table('level2');",
      );
      await writeFile(
        join(level3, "level3.sql"),
        "SELECT has_table('level3');",
      );
  ...
  describe("Error handling", () => {
    it("should throw error for non-existent directory", async () => {
      ...
      await expect(scanner.scanDirectory(nonExistentDir)).rejects.toThrow(
        "ENOENT",
      );
    });
    ...
    it("should throw error for file instead of directory", async () => {
      ...
      await expect(scanner.scanDirectory(testFile)).rejects.toThrow(
  ...
  describe("Integration with parseTestFile", () => {
    it("should integrate discovered files with parsing", async () => {
      ...
      const totalAssertions = testFiles.reduce(
        (sum, file) => sum + file.assertions.length,
        0,
      );
    ...
    it("should handle files with parsing errors gracefully", async () => {
      ...
      await writeFile(
        join(tempDir, "invalid.sql"),
        "SELECT has_table('test');",
      );
