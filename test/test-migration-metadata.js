...
const MigrationMetadata = require("../src/lib/MigrationMetadata");
...
class MigrationMetadataTests {
  ...
  testConstructor() {
    ...
    this.assert(
      metadata.migrationPath === migrationPath,
      "Constructor sets migration path",
    );
  ...
  testCreateDefault() {
    ...
    this.assert(
      ...
      "createDefault sets pending status",
    );
    this.assert(
      metadata.testing.tested_at === null,
      "createDefault initializes testing",
    );
    this.assert(
      metadata.promotion.promoted_at === null,
      "createDefault initializes promotion",
    );
    this.assert(
      typeof metadata.generated === "string",
      "createDefault sets generated timestamp",
    );
  ...
  testReadWrite() {
    ...
    const testData = MigrationMetadata.createDefault(
      "20250828_123456",
      "test_migration",
    );
    ...
    this.assert(
      fs.existsSync(metadata.metadataFile),
      "Write creates metadata file",
    );
    ...
    this.assert(
      readData.status === testData.status,
      "Read returns correct status",
    );
    ...
    fs.writeFileSync(
      path.join(invalidJsonPath, "metadata.json"),
      "{ invalid json }",
    );
  ...
  testUpdate() {
    ...
    const initial = MigrationMetadata.createDefault(
      "20250828_123456",
      "test_migration",
    );
    ...
    this.assert(
      nestedUpdate.testing.tests_failed === 0,
      "Update preserves nested fields",
    );
  ...
  testLifecycle() {
    ...
    this.assert(
      ...
      "Lifecycle reaches promoted status",
    );
    this.assert(
      final.testing.tests_passed === 12,
      "Lifecycle preserves test results",
    );
    this.assert(
      final.promotion.promoted_by === "admin@example.com",
      "Lifecycle tracks promotion",
    );

    // Verify file persistence
    const reread = metadata.read();
    this.assert(
      reread.status === "promoted",
      "Lifecycle changes persist to disk",
    );
  }
...
module.exports = MigrationMetadataTests;
